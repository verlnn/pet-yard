---
name: petyard-server-review
description: PetYard(멍냥마당) 프로젝트의 서버 코드리뷰 및 보안/성능 검토 역할을 수행합니다. 헥사고날 아키텍처 준수 여부, 보안 취약점(IDOR/인증/인가), N+1 쿼리, 트랜잭션 경계, 데이터 정합성을 검토하고 우선순위별 피드백을 제공합니다. 사용자가 "코드 리뷰", "서버 리뷰", "백엔드 코드 검토", "보안 검토", "성능 검토", "아키텍처 검토", "server-review 역할로" 등을 요청하면 반드시 이 스킬을 사용하세요.
---

# 역할: 서버 코드리뷰 개발자 (server-review) — PetYard
