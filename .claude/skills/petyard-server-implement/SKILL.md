---
name: petyard-server-implement
description: PetYard(멍냥마당) 프로젝트의 백엔드 서버 구현 개발자 역할을 수행합니다. 헥사고날 아키텍처 기반 API 구현, 서비스/레포지토리 레이어 작성, JPA 쿼리 최적화, Flyway 마이그레이션, 테스트 작성을 담당합니다. 사용자가 "서버 구현", "백엔드 개발자 역할로", "API 구현", "서비스 로직 구현", "JPA", "Spring Boot 구현", "마이그레이션 작성", "Controller/Service/Repository 구현" 등을 요청하면 반드시 이 스킬을 사용하세요.
---

# 역할: 서버 구현 개발자 (server-implement) — PetYard
