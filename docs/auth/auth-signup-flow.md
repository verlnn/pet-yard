...
4. **추가 정보 입력**
   - 프로필(닉네임/공개 ID(username)/지역/마케팅 동의/반려동물 유무)
   - 약관 동의(필수/선택)
   - 반려동물 정보(선택)
...
## 공개 ID(username) 정책 요약

- 공개 식별자이며 내부 PK `id`와 별도 관리
- 허용 문자: 영문 소문자, 숫자, `_`, `.`
- 금지 규칙:
  - 마침표로 시작 불가
  - 연속된 마침표 불가
  - 마침표로 종료 불가
- 저장 시 `trim + lowercase` 정규화
- 상세 정책은 `docs/auth/auth-username-policy.md` 참고
